...
> ## ⭐ 빠른 사용 — `nosim.cmd` 하나로 끝
> repo 루트의 **`nosim.cmd` 를 더블클릭**하면 관리 콘솔(메뉴)이 뜬다. 일상 운영은 이거 하나면 된다:
>
> | 메뉴 | 하는 일 |
> |---|---|
> | **1) 배포** | 최신 코드 받기 (`git pull` + 필요 시 `npm install` 자동) |
> | **2) 자동수집 켜기** | 작업 스케줄러 등록 (30분마다) |
> | **3) 자동수집 끄기** | 작업 해제 |
> | **4) 지금 수집** | 현재 영업일 1회 즉시 수집 (테스트) |
> | **5) 점검만** | 로그인/계정조회만 (dry-run, 수집 안 함) |
> | **6) 상태 보기** | 작업 상태·코드 버전·오늘 로그·쿠팡 쿨다운 한눈에 |
> | **7) 쿠팡 쿨다운 초기화** | 차단 backoff 강제 해제 (40분 쉰 뒤에만) |
>
> 한글 로그가 안 깨지도록 콘솔을 UTF-8 로 맞추고, 단계마다 색상으로 OK/경고/오류를 구분한다.
> 아래 §1~§4 는 **맨바닥 새 PC 최초 1회 세팅** 설명 — 세팅이 끝나면 `nosim.cmd` 만 쓰면 된다.
>
> CLI 로 개별 동작만 부르고 싶으면:
> ```powershell
> powershell -ExecutionPolicy Bypass -File scripts\collect-control.ps1 -Action deploy   # 배포만
> powershell -ExecutionPolicy Bypass -File scripts\collect-control.ps1 -Action status   # 상태만
> ```
...
가장 쉬운 길: **`nosim.cmd` 더블클릭 → `2) 자동수집 켜기`**. (아래는 동등한 수동 경로)
